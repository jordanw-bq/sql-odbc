...
def RunTests(tests, test_type):
    ...
    for test in tests:
        ...
        total_failures += subprocess.call([test,
                                           "-fout", output_path, "--gtest_color=no"])
        if test_type == UT_TYPE:
            with open(output_path, "r+") as f:
                output.append({"UnitTest": test.split(os.path.sep)
                               [-1].replace(".exe", ""), "Log": f.read()})
        elif test_type == IT_TYPE:
            with open(output_path, "r+") as f:
                output.append({"IntegrationTest": test.split(
                    os.path.sep)[-1].replace(".exe", ""), "Log": f.read()})
...
def GetAndTranslatePerformanceInfo(test):
    ...
    if output == {}:
        raise Exception(
            "Failed to get any information out of performance info log")
    return output
...
def GetAndTranslatePerformanceResults(test):
    ...
    output = []
    single_case = {}
    sync_items_line = [SYNC_QUERY, SYNC_CASE,
                       SYNC_MIN, SYNC_MAX, SYNC_MEAN, SYNC_MEDIAN]
    sync_items_readable = [item.replace("%%", "").replace(
        "__", "").capitalize() for item in sync_items_line]
    for line in log:
        if SYNC_START in line:
            single_case = {}
            reading = True
        ...
        if reading:
            for sync_item in sync_items_line:
                if sync_item in line:
                    single_case[sync_item.replace("%%", "").replace(
                        "__", "").capitalize()] = line.replace(sync_item, "").rstrip()
    return output
...
def ParseUnitTestCase(log_lines, test_case):
    ...
    for log_line in log_lines:
        ...
        if "[----------] " in log_line and (test_case + " ") in log_line and log_line.endswith(" ms total)"):
            test_case_info["TotalTime"] = FindBetween(
                log_line, "(", ")").replace(" total", "")
    ...
    for test in tests:
        ...
        for log_line in log_lines:
            if test_tag in log_line and log_line.endswith(")"):
                ...
                test_info["TestResult"] = FindBetween(
                    log_line, "[", "]").replace(" ", "")
...
def ParseUnitTestLog(unit_test, log):
    ...
    for log in log_split:
        if log.startswith("[==========] Running"):
            tmp = log.replace("[==========] Running ", "").replace(" test cases.", "").replace(
                " test case.", "").replace("tests from", "").replace("test from", "")
    if tmp == "":
...
def TranslateTestOutput(test_type, outputs):
    ...
    if test_type == UT_TYPE:
        for output in outputs:
            log_jsons.append(ParseUnitTestLog(
                output["UnitTest"], output["Log"]))
    elif test_type == IT_TYPE:
        for output in outputs:
            log_jsons.append(ParseUnitTestLog(
                output["IntegrationTest"], output["Log"]))
    return log_jsons
        ...
        if PERFORMANCE_TYPE == _type:
            ...
            for test in tests:
                if test.replace(".exe", "").endswith(PERFORMANCE_INFO):
                    final_output[PERFORMANCE_TYPE]["Info"] = GetAndTranslatePerformanceInfo(
                        test)
                elif test.replace(".exe", "").endswith(PERFORMANCE_RESULTS):
                    final_output[PERFORMANCE_TYPE]["Results"] = GetAndTranslatePerformanceResults(
                        test)
        else:
...
def main():
    try:
        ...
        if infile is None or outfile is None:
            print(
                "Usage: -i <infile> -o <outfile> [-s <test_suites> -e <efile>]")
            sys.exit(1)
        exclude_test_list = []
        global total_failures
        total_failures = 0
        if efile is not None:
            with open(efile) as ef:
                ...
                exclude_test_list = [l.strip()
                                     for l in exclude_test_list if l.strip() != ""]
        ...
        with open(os.path.join(os.getcwd(), outfile), 'w+') as results_file:
            data = RunAllTests(
                [UT_TYPE, IT_TYPE, PERFORMANCE_TYPE], suites, exclude_test_list)
